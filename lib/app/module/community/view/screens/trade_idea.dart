import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:get/get_core/src/get_main.dart';
import 'package:statifi/app/module/community/controller/trade_idea_controller.dart';
import 'package:statifi/app/module/community/view/screens/sub_widgets/rich_text_switch.dart';
import 'package:statifi/core/constants/app_colors.dart';
import 'package:statifi/core/extentions/media_query_extension.dart';
import 'package:statifi/global_widgets/app_textstyle.dart';
import 'package:statifi/global_widgets/appbar_header.dart';
import 'package:statifi/global_widgets/auth_button.dart';
import 'package:statifi/global_widgets/custom_textfield.dart';

import '../../../../../global_widgets/app_background.dart';

class TradeIdea extends StatelessWidget {
  const TradeIdea({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(TradeIdeaController());

    return Scaffold(
      body: SafeArea(
        child: Container(
          height: context.screeHeight,
          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
          decoration: BoxDecoration(gradient: AppBackground.splashGradient),
          child: SingleChildScrollView(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                AppbarHeader(title: "Trade Idea"),

                const SizedBox(height: 20),

                Text("Title", style: AppTextStyles.body),
                const SizedBox(height: 10),
                CustomTextField(hintText: "enter your trade title..."),

                const SizedBox(height: 15),

                Text("Content", style: AppTextStyles.body),
                const SizedBox(height: 10),
                CustomTextField(
                  hintText: "Share your trade idea in detail...",
                  maxLine: 4,
                ),

                const SizedBox(height: 10),
                Divider(color: AppColors.tColor1),
                const SizedBox(height: 10),

                Text("Symbol", style: AppTextStyles.body),
                const SizedBox(height: 10),
                CustomTextField(hintText: "e.g. EUR, USD"),

                const SizedBox(height: 15),

                Text("Type", style: AppTextStyles.body),
                const SizedBox(height: 10),
                CustomTextField(
                  hintText: "Buy",
                  postfixIcon: Icons.keyboard_arrow_down,
                  hintColor: Colors.white,
                  iconColor: Colors.white,
                ),

                const SizedBox(height: 15),

                Row(
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Entry price", style: AppTextStyles.body),
                          const SizedBox(height: 8),
                          CustomTextField(hintText: "10.5000"),
                        ],
                      ),
                    ),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Stop loss", style: AppTextStyles.body),
                          const SizedBox(height: 8),
                          CustomTextField(hintText: "10.5000"),
                        ],
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 15),
                Row(
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Take Profit", style: AppTextStyles.body),
                          const SizedBox(height: 8),
                          CustomTextField(hintText: "10.5000"),
                        ],
                      ),
                    ),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Lot Size", style: AppTextStyles.body),
                          const SizedBox(height: 8),
                          CustomTextField(hintText: "0.1"),
                        ],
                      ),
                    ),
                  ],
                ),

                const SizedBox(height: 15),

                Text("Trade reasoning", style: AppTextStyles.body),
                const SizedBox(height: 10),
                CustomTextField(
                  hintText: "explain your reasoning...",
                  maxLine: 4,
                ),

                const SizedBox(height: 15),

                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text("Tags (Optional)", style: AppTextStyles.body),
                    Text(
                      "+ add more tags",
                      style: AppTextStyles.body.copyWith(
                        color: AppColors.btnColor1,
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 10),
                CustomTextField(
                  hintText: "# add tags (e.g. EURUSD, breakout, scalping)",
                ),
                const SizedBox(height: 10),
                Text(
                  "Add upto 5 tags to help others find your post)",
                  style: AppTextStyles.body.copyWith(color: Colors.white30),
                ),
                const SizedBox(height: 10),
                Divider(color: AppColors.tColor1),
                const SizedBox(height: 15),

                // Container here
                // const RichTextSwitchContainer(),
                RichTextSwitchContainer(
                  title: "Make post public",
                  subtitle: "Public post are visible to all community members.",
                  switchValue: controller.isExtraInfoEnabled,
                  onChanged: (value) {
                    controller.isExtraInfoEnabled.value = value;
                  },
                ),

                const SizedBox(height: 20),

                AuthButton(text: "Post now", onTap: () {}),
              ],
            ),
          ),
        ),
      ),
    );
  }
}
